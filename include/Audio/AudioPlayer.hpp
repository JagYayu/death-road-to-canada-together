...
namespace tudov
{
	...
	struct PlayAudioArgs
	{
		...
		std::float_t pitch = 1.0f;              // 音调（1.0 = 原速）
		std::float_t gain = 1.0f;               // 音量（1.0 = 原音量）
		std::float_t rolloffFactor = -1.0f;     // 衰减系数（距离影响音量）
		std::float_t referenceDistance = -1.0f; // 声音开始衰减的参考距离
		std::float_t maxDistance = -1.0f;       // 最大距离（之后不再衰减）
	};
